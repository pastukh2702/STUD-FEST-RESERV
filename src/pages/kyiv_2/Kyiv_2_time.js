...
const ZONES = ['Management Zone (Comfy)', 'Hackaton Zone'];
let TIMES = [];
...
export class Kyiv_2_time extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount(){
        TIMES = [];
        getRegistrations("kyiv-2-lec").then((res) => {
            res.forEach((doc) => {
                if(doc.data().zone == 1){
                    if(doc.data().about == ''){
                        TIMES.push({
