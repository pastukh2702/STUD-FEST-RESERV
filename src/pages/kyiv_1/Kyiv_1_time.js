...
const ZONES = ['Managment Zone (COMFY)', 'Tech Zone (lifecell)', 'Free Zone'];
let TIMES = [];
...
export class Kyiv_1_time extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount(){
        TIMES = [];
        getRegistrations("kyiv-1-lec").then((res) => {
            res.forEach((doc) => {
                if(doc.data().zone == 1){
                    if(doc.data().about == ''){
                        TIMES.push({
