...
const ZONES = ['M Zone', 'IT Zone', 'Hackaton Zone'];
let TIMES = [];
...
export class Dnipro_time extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount(){
        TIMES = [];
        getRegistrations("dnipro-lec").then((res) => {
            res.forEach((doc) => {
                if(doc.data().zone == 1){
                    if(doc.data().about == ''){
                        TIMES.push({
